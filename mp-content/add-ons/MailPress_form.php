...
$mp_file = 'admin.php';
define ( 'MailPress_forms',		add_query_arg( 'page', MailPress_page_forms, 	$mp_file ) );
define ( 'MailPress_fields',		add_query_arg( 'page', MailPress_page_fields, 	$mp_file ) );
define ( 'MailPress_templates',		add_query_arg( 'page', MailPress_page_templates,$mp_file ) );
	...
	public static function mp_action_add_form() 
	{
		...
		if ( '' === trim( $_POST['label'] ) )
		{
			$x = new WP_Ajax_Response( array( 	'what' => 'form', 
									'id' => new WP_Error( 'label', __( 'You did not enter a valid label.', 'MailPress' ) )
								   ) );
			$x->send();
		}
		...
		{
			$x = new WP_Ajax_Response( array( 	'what' => 'form', 
									'id' => new WP_Error( 'settings[recipient][toemail]', __( 'You did not enter a valid email.', 'MailPress' ) )
								   ) );
			$x->send();
		}
...
// for preview
	public static function mp_action_ifview()
	{
		...
		$form_url = esc_url( admin_url( add_query_arg( array('action' => 'edit', 'id' => $form->id ), MailPress_forms ) ) );
		$field_url = esc_url( admin_url( add_query_arg( 'form_id', $form->id, MailPress_fields ) ) );
		$template_url = esc_url( admin_url( add_query_arg( 'template', $form->template, MailPress_templates ) ) );
	...
	{
		$form = MP_Form::get( $_GET['id'] );
		global $title; $title = sprintf( __( 'Preview "%1$s"', 'MailPress' ), $form->label );
	}
}
new MailPress_form();
}