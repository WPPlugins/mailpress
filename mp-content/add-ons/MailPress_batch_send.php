...
class MailPress_batch_send
{
	...
	const process_name = 'mp_process_batch_send';
...
// for settings batches
	...
	{
		include ( MP_ABSPATH . 'mp-admin/includes/settings/batches/batch_send/help.php' );
		return $content;
	}

	public static function settings_batches_update()
	{
		include ( MP_ABSPATH . 'mp-admin/includes/settings/batches/batch_send/update.php' );
	}

	public static function settings_batches_form()
	{
		include ( MP_ABSPATH . 'mp-admin/includes/settings/batches/batch_send/form.php' );
	}

// for settings logs
	public static function settings_logs_form( $logs )
	{
		MP_AdminPage::logs_sub_form( self::log_name, $logs, __( 'Batch', 'MailPress' ) );
	}
...
// for meta box in tracking page
	public static function tracking_add_meta_box( $screen )
	{
		...
		add_meta_box( 'batchsenddiv', __( 'Batch current status', 'MailPress' ), array( __CLASS__, 'meta_box_status' ), $screen, 'normal', 'core' );
	}
...
?>
		<table style="width:100%;">
			<tr>
				<td><?php _e( 'Batch status', 'MailPress' ); ?></td>
				<td colspan="3"><?php echo $mail->status; ?></td>
			</tr>
<?php 	if ( is_array( $mailmeta ) ) : ?>
			<tr><td colspan="4">&#160;</td></tr>
			<tr>
				<td style="width:25%;text-align:center;"><?php _e( 'Total recipients', 'MailPress' ); ?></td>
				<td style="width:25%;text-align:center;"><?php _e( 'Sent', 'MailPress' ); ?></td>
				<td style="width:25%;text-align:center;"><?php _e( 'Try/Pass', 'MailPress' ); ?></td>
				<td style="width:25%;text-align:center;"><?php _e( 'Processed', 'MailPress' ); ?></td>
			</tr>
			<tr>
...
<?php 		if ( !empty( $mailmeta['failed'] ) ) : ?>
			<tr><td colspan="4">&#160;</td></tr>
			<tr>
				<td><?php printf( __( 'Failed (%1$s)', 'MailPress' ), count( $mailmeta['failed'] ) ); ?></td>
