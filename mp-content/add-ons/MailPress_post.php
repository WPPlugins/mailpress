...
// for ajax in write mail
		add_action( 'wp_ajax_order_mppost',	array( __CLASS__, 'wp_ajax_order_mppost' ) );
		add_action( 'mp_action_delete_mppost',array( __CLASS__, 'wp_ajax_delete_mppost' ) );
...
// for mails list
	...
	{ 
		if ( MP_Post::object_have_relations( $mail_id ) ) $out .= '<span class="icon post" title="' . esc_attr( __( 'Posts', 'MailPress' ) ) . '"></span>';
		return $out;
	}
...
// for meta box in write post  ////
	public static function add_help_tab_write()
	{
		...
		$content .= '<li>' . sprintf( __( 'Draft mail with posts can be quickly identified in %1$s with a little pin icon : %2$s', 'MailPress' ), sprintf( '<a href="' . MailPress_mails . '" target="_blank">%s</a>', __( 'Mails list', 'MailPress' ) ) , '<span class="icon post" title="' . esc_attr( __('Posts', 'MailPress' ) ) . '" style="height:16px;width:16px;padding:0;margin:0 0 0 3px;display:inline-block;background:url(\'' . site_url() . '/' . MP_PATH . 'mp-admin/images/list_icons.png' . '\') no-repeat scroll -48px 0 transparent;"></span>' ) . '</li>';
		$content .= '</ul>';
	...
	public static function get_draft_row( $id, $subject )
	{
		...
		$actions['delete'] = '<a class="delete:mpdraftchecklist:mpdraft-' . $id . '" data-wp-lists="delete:mpdraftchecklist:mpdraft-' . $id . '" href="' . $delete_url . '" title="' . esc_attr( __( 'Delete link', 'MailPress' ) ) . '"><img src="' . site_url() . '/' . MP_PATH . 'mp-admin/images/trash.png" alt="" /></a>';
...
<?php
	...
	public static function get_post_row( $id )
	{
		...
		$actions['sortable'] = '<img class="mppost-handle" style="cursor:move" alt="' . esc_attr( __( 'up/down', 'MailPress' ) ) . '" title="' . esc_attr( __( 'up/down', 'MailPress' ) ) . '" src="' . site_url() . '/' . MP_PATH . 'mp-admin/images/sortable.png" alt="" />';

		$edit_url = esc_url( add_query_arg( array( 'action' => 'edit', 'post' => $id ) , 'post.php' ) );
		$actions['edit'] = '<a href="' . $edit_url . '" title="' . esc_attr( sprintf( __( 'Edit &#8220;%s&#8221;', 'MailPress' ) , $ptitle ) ) . '">' . $id . '</a>';
		...
		$actions['delete'] = '<a class="delete:mppostchecklist:mppost-' . $id . '" href="' . $delete_url . '" title="' . esc_attr( __( 'Delete link', 'MailPress' ) ) . '"><img src="' . site_url() . '/' . MP_PATH . 'mp-admin/images/trash.png" alt="" /></a>';
	...
	public static function wp_ajax_order_mppost()
	{
            $mp_mail_id = $_POST['id'];
	...
	public static function wp_ajax_delete_mppost()
	{
