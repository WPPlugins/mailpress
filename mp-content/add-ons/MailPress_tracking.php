...
// prepare mail
	...
	public static function mail( $mail )
	{
		...
		$output = preg_match_all( '/<a [^>]*href=[\'"](.*?)[\'"]([^>]*)>/si', $mail->html, $matches, PREG_SET_ORDER );     /* was '/<a [^>]*href=[\'"]([^\'"]+)[\'"][^>]*>(.*?)<\/a>/is' */
...
// process link
	...
	public static function save( $meta )
	{
		...
		$data['ip'] 		= $wpdb->_real_escape( trim( $_SERVER['REMOTE_ADDR'] ) );								$format[] = '%s';
		$data['agent'] 		= $wpdb->_real_escape( trim( $_SERVER['HTTP_USER_AGENT'] ) );							$format[] = '%s';
		$data['referrer']		= ( isset( $_SERVER['HTTP_REFERER'] ) ) ? $wpdb->_real_escape( trim( $_SERVER['HTTP_REFERER'] ) ) : '';	$format[] = '%s';
		$data['tmstp']		= current_time( 'mysql' );													$format[] = '%s';
