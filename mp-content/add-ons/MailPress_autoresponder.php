...
// for settings
	...
	{
		MP_AdminPage::logs_sub_form( self::log_name, $logs, __( 'Autoresponder', 'MailPress' ) );
	}
...
// for mails list
	...
	{
		if ( MP_Autoresponder::object_have_relations( $mail_id ) ) $out .= '<span class="icon autoresponder" title="' . __( 'Autoresponder', 'MailPress' ) . '"></span>';
		return $out;
	}
...
// for meta box in write page
	public static function add_help_tab_write()
	{
		...
		$content .= '<li>' . sprintf( __( 'Autoresponder draft mail can be quickly identified in %1$s with a little clock icon : %2$s', 'MailPress' ), sprintf( '<a href="' . MailPress_mails . '" target="_blank">%s</a>', __( 'Mails list', 'MailPress' ) ) , '<span class="icon autoresponder" title="' . __('Autoresponder', 'MailPress' ) . '" style="height:16px;width:16px;padding:0;margin:0 0 0 3px;display:inline-block;background:url(\'' . site_url() . '/' . MP_PATH . 'mp-admin/images/list_icons.png' . '\') no-repeat scroll -32px 0 transparent;"></span>' ) . '</li>';
		$content .= '</ul>';
		...
		<tbody>
			<tr>
				<td id="newarleft" class="left">
					<select name="autoresponderselect" id="autoresponderselect" tabindex="7">
				...
				<td style="vertical-align:top;">
					<table style="border:none;margin:8px 0 8px 8px;width:95%;">
						<tbody>
							<tr>
								<td class="arschedule">
									<?php _e( 'Year', 'MailPress' );?><br />
									<select name="autoresponder[schedule][Y]" >
<?php MP_AdminPage::select_number( 0, 99, 0 ); ?>
									</select>
								</td>
								<td class="arschedule">
									<?php _e( 'Month', 'MailPress' );?><br />
									<select name="autoresponder[schedule][M]" >
<?php MP_AdminPage::select_number( 0, 99, 0 ); ?>
									</select>
								</td>
								<td class="arschedule">
									<?php _e( 'Week', 'MailPress' );?><br />
									<select name="autoresponder[schedule][W]" >
<?php MP_AdminPage::select_number( 0, 99, 0 ); ?>
									</select>
								</td>
								<td class="arschedule">
									<?php _e( 'Day', 'MailPress' );?><br />
									<select name="autoresponder[schedule][D]" >
<?php MP_AdminPage::select_number( 0, 99, 0 ); ?>
									</select>
								</td>
								<td class="arschedule">
									<?php _e( 'Hour', 'MailPress' );?><br />
									<select name="autoresponder[schedule][H]" >
<?php MP_AdminPage::select_number( 0, 99, 0 ); ?>
									</select>
								</td>
							</tr>
			...
			<tr>
				<td>
					<div class="submit" style="border: 0 none;float: none;padding: 0 8px 8px;">
						<input type="submit" name="addwrite_autoresponder" id="addmetasub2" class="add:<?php echo $parms['table_body_id']; ?>:add_<?php echo $parms['tr_prefix_id']; ?> button" tabindex="9" value="<?php _e( 'Add', 'MailPress' ) ?>" />
					</div>
				</td>
				<td>
					<p class="description"><?php _e( 'Not more than one year between two scheduled mails.', 'MailPress' ); ?></p>
...
<?php
	...
	public static function meta_box_autoresponder_row( $entry, &$count ) 
	{
		...
		$out .= MP_::select_option( $_autoresponders, $entry['term_id'], false );
		$out .= '</select>';
		...
		$out .= __( 'Year', 'MailPress' ) . '<br />';
		$out .= '<select name="write_autoresponder[' . $entry['meta_id'] . '][value][Y]">';
		$out .= MP_::select_number( 0, 99, $entry['schedule']['Y'], 1, false );
		$out .= '</select>';
		$out .= '</td>';
		$out .= '<td class="arschedule">';
		$out .= __( 'Month', 'MailPress' ) . '<br />';
		$out .= '<select name="write_autoresponder[' . $entry['meta_id'] . '][value][M]">';
		$out .= MP_::select_number( 0, 99, $entry['schedule']['M'], 1, false );
		$out .= '</select>';
		$out .= '</td>';
		$out .= '<td class="arschedule">';
		$out .= __( 'Week', 'MailPress' ) . '<br />';
		$out .= '<select name="write_autoresponder[' . $entry['meta_id'] . '][value][W]">';
		$out .= MP_::select_number( 0, 99, $entry['schedule']['W'], 1, false );
		$out .= '</select>';
		$out .= '</td>';
		$out .= '<td class="arschedule">';
		$out .= __( 'Day', 'MailPress' ) . '<br />';
		$out .= '<select name="write_autoresponder[' . $entry['meta_id'] . '][value][D]">';
		$out .= MP_::select_number( 0, 99, $entry['schedule']['D'], 1, false );
		$out .= '</select>';
		$out .= '</td>';
		$out .= '<td class="arschedule">';
		$out .= __( 'Hour', 'MailPress' ) . '<br />';
		$out .= '<select name="write_autoresponder[' . $entry['meta_id'] . '][value][H]">';
		$out .= MP_::select_number( 0, 99, $entry['schedule']['H'], 1, false );
		$out .= '</select>';
		$out .= '</td>';
		$out .= '</tr>';
		$out .= '</tbody>';
