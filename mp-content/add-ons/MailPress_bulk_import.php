...
<form id="bulk-add" method="post" style="z-index:2000">
	<input type="text"   name="emails" id="emails" onclick="document.getElementById( 'bulk-add' ).style.width='90%';document.getElementById( 'emails' ).style.width='70%';document.getElementById( 'radios' ).style.display='block';" value="" />
	<input type="submit" name="bulk_add" class="button" value="<?php _e( 'Bulk Add', 'MailPress' ); ?>" />
	<div id="radios" style="display:none">

<?php if ( current_user_can( 'MailPress_manage_mailinglists' ) ) MP_Mailinglist::dropdown( array( 'htmlname' => 'bulk_import_mailinglist', 'htmlid' => 'bulk_import_mailinglist', 'hierarchical' => true, 'orderby' => 'name', 'hide_empty' => '0', 'show_option_none' => __( 'Choose mailinglist', 'MailPress' ) ) ); ?>

		<label for="bulki-activate"><input type="radio" name="activate" id="bulki-activate" value="activate" /><?php _e( 'Activate', 'MailPress' ); ?></label>
		<label for="bulki-waiting"><input  type="radio" name="activate" id="bulki-waiting"  value="waiting" checked="checked" /> <?php _e( 'Require Authorization', 'MailPress' ); ?></label>
