...
// for wp admin
		...
		// for settings general
			add_filter( 'MailPress_settings_general_help',	array( __CLASS__, 'settings_general_help' ), 20, 1 );
			add_action( 'MailPress_settings_general_update',array( __CLASS__, 'settings_general_update' ) );
			add_action( 'MailPress_settings_general_form',	array( __CLASS__, 'settings_general_form' ) );
...
// for settings general
	...
	{
		include ( MP_ABSPATH . 'mp-admin/includes/settings/general/sync_wordpress_user/help.php' );
		return $content;
	}

	public static function settings_general_update()
	{
		include ( MP_ABSPATH . 'mp-admin/includes/settings/general/sync_wordpress_user/update.php' );
	}

	public static function settings_general_form()
	{
		include ( MP_ABSPATH . 'mp-admin/includes/settings/general/sync_wordpress_user/form.php' );
	}

// for meta box in user page
	public static function meta_boxes_user( $mp_user_id, $mp_screen )
	{
		add_meta_box( 'mp_user_syncwordpress', __( 'WP User sync', 'MailPress' ) , array( __CLASS__, 'meta_box' ), MP_AdminPage::screen, 'side', 'core' );
	}
...
?>
		<tr><td style="padding:0px;" colspan="2"><strong><?php printf( __( 'WP User # %1$s', 'MailPress' ), $wp_user->ID ); ?></strong></td></tr>
