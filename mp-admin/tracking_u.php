...
////  Row  ////
	...
	{
		add_action( 'MailPress_get_icon_users', 	array( __CLASS__, 'get_icon_users' ), 8, 2 ); // just one row !
...
//	class
	...
	public static function get_flag_IP() {
		global $mp_user;
		return ( ( 'ZZ' == $mp_user->created_country ) || empty( $mp_user->created_country ) ) ? '' : '<img class="flag" alt="' . esc_attr( strtolower( $mp_user->created_country ) ) . '" title="' . esc_attr( strtolower( $mp_user->created_country ) ) . '" src="' . site_url() . '/' . MP_PATH . 'mp-admin/images/flag/' . strtolower( $mp_user->created_country ) . '.gif" />';
	}
	...
	public static function get_icon_users( $out, $mp_user )
	{
		...
		{
			$out .= '<span class="icon unsubscribed" title="' . esc_attr( __( 'Unsubscribed', 'MailPress' ) ) . '"></span>';
		}
		return $out;
	}
}