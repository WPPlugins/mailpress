...
// messages
$message = ''; $err = 0;
if ( isset( $_GET['saved'] ) ) 	{$err += 0; if ( !empty( $message ) ) $message .= '<br />'; $message .= __( 'User saved', 'MailPress' ); }
...
<?php if ( $message ) MP_AdminPage::message( $message, ( $err ) ? false : true ); ?>
	<form id="mp_user" name="mp_user_form" method="post">
		...
		<input type="hidden" name="id" value="<?php echo $mp_user->id ?>" />
					...
					<table class="form-table">
						<tbody>
							<tr>
								...
								<td class="h1em">
									<input type="text" size="30" value="<?php echo $mp_user->email; ?>" disabled="disabled" />
								</td>
...
<?php echo MP_User::get_flag_IP(); ?>
							...
							<tr>
								...
								<td class="h1em">
									<input type="text" name="mp_user_name" size="30" value="<?php echo esc_attr( $mp_user->name ); ?>" />
