...
if ( isset( $draft ) )
{
	...
	$hidden		= '<input type="hidden" name="mail_id" id="mail_id" value="' . $draft->id . '" />';
...
if ( isset( $_GET['sched'] ) ) 	{$err += 0; if ( !empty( $message ) ) $message .= '<br />'; $message .= sprintf( __( 'Mail scheduled for: <strong>%1$s</strong>. <a class="thickbox thickbox-preview" href="%2$s">Preview mail</a>',  'MailPress' ), date_i18n( __( 'M j, Y @ G:i' ), strtotime( $draft->sent ) ), $view_url );}
...
if ( isset( $_GET['sent'] ) ) 	{$err += 0; if ( !empty( $message ) ) $message .= '<br />'; $message .= sprintf( _n( __( '%s mail sent', 'MailPress' ), __( '%s mails sent', 'MailPress' ), $_GET['sent'] ), $_GET['sent'] ); }
if ( isset( $_GET['revision'] )){$err += 0; if ( !empty( $message ) ) $message .= '<br />'; $message .= sprintf( __( 'Mail restored to revision from %s', 'MailPress' ), MP_Mail_revision::title(  MP_Mail::get( ( int ) $_GET['revision'] ), false ) ); }
...
<?php MP_AdminPage::select_optgroup( $draft_dest, ( isset( $draft->to_list ) ) ? $draft->to_list : '' ) ?>
								...
								<div id="toemail-toname" style="padding-bottom:5px;<?php if ( isset( $draft->to_list ) ) echo "display:none;";?>">
									<label for="toemail" id="toemail-prompt-text" class="hide-if-no-js" style="color:#bbb;padding:6px 0 0 8px;position:absolute;<?php if ( esc_attr( $draft->toemail ) != '' ) echo 'visibility:hidden;' ; ?>"><?php _e( 'Enter email here', 'MailPress' ); ?></label>
									<input type="text" name="toemail" id="toemail" title="<?php echo esc_attr( __( 'Email', 'MailPress' ) ); ?>" class="<?php echo $class; ?>" style="width:45%;" value="<?php echo esc_attr( $draft->toemail ); ?>" />
									<label for="toname"  id="toname-prompt-text"  class="hide-if-no-js" style="color:#bbb;padding:6px 0 0 8px;position:absolute;<?php if ( esc_attr( $draft->toname  ) != '' ) echo 'visibility:hidden;' ; ?>"><?php _e( 'Enter name here',  'MailPress' ); ?></label>
									<input type="text" name="toname"  id="toname"  title="<?php echo esc_attr( __( 'Name', 'MailPress' ) ); ?>"  class="<?php echo $class; ?>" style="width:45%;" value="<?php echo esc_attr( $draft->toname ); ?>" />
								</div>
							</td></tr></table>
					...
					<div id="titlediv">
						<div id="titlewrap">
							<label for="title" id="title-prompt-text" class="hide-if-no-js" style="<?php echo ( isset( $draft->subject ) ) ? 'visibility:hidden' : ''; ?>"><?php _e( 'Enter subject here', 'MailPress' ); ?></label>
