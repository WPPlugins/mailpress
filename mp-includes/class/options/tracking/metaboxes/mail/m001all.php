	...
	function meta_box( $mail )
	{
		...
		if ( $tracks ) 
		{
			...
			foreach( $tracks as $track ) 
			{
				...
				echo '<tr><td><abbr title="' . esc_attr( $track->tmstp ) . '">' . substr( $track->tmstp, 0, 10 ) . '</abbr></td><td>&#160;' . $action . '</td><td>&#160;' . MailPress_tracking::translate_track( $track->track, $track->mail_id ) . '</td></tr>';
			}
