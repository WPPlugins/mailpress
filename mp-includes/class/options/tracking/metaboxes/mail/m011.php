...
class MP_Tracking_metabox_m011 extends MP_tracking_metabox_
{
	...
	function meta_box( $mail )
	{
		...
		if ( is_email( $mail->toemail ) ) $m[$mail->toemail] = array( '{{_user_id}}' => MP_User::get_id_by_email( $mail->toemail ) );
		...
		echo '<table id ="tracking_mp_011">';
		echo '<tr><th>' . __( 'domain', 'MailPress' ) . '</th><th class="num">' . __( 'count', 'MailPress' ) . '</th><th class="num">' . __( '%', 'MailPress' ) . '</th><th class="num">' . __( 'open rate', 'MailPress' ) . '</th></tr>';
		foreach( $x as $k => $v )
		{
