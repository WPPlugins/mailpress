...
class MP_Tracking_metabox_u009 extends MP_tracking_metabox_
{
	...
	function meta_box( $mp_user )
	{
		...
		$dend = $wpdb->get_var( $wpdb->prepare( "SELECT max( DATE( tmstp ) ) FROM $wpdb->mp_tracks WHERE user_id = %d AND mail_id <> 0 ;", $mp_user->id ) );
                if ( !$dend ) return;
                
		$m = substr( $dend,5,2 ); 
		$d = substr( $dend,8,2 );
		$y = substr( $dend,0,4 ); 
		$dbeg = date( 'Y-m-d', mktime( 0, 0, 0, $m, $d-65, $y ) );
		...
		$args['chds'] = "0,{$chxl_y}";
		$args['chdlp']= 'b';
		$args['chdl'] = __( 'Opened', 'MailPress' ) . '|' . __( 'Clicks', 'MailPress' );
...
<img style="width:100%;" src="<?php echo $url; ?>" alt="<?php _e( 'Mails - send', 'MailPress' ); ?>" /></div>
<?php
	}
}
new MP_Tracking_metabox_u009( __( 'Opened, Clicks/day chart', 'MailPress' ) );