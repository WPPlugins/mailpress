...
class MP_Form_field_type_geotag extends MP_form_field_type_
{
	...
	{
		return ( isset( $field->settings['options']['is'] ) ) ? $this->prefix . '[' . $field->form_id . ']['. $field->id . '][' . $field->settings['options']['is'] . ']' : $this->prefix . '[' . $field->form_id . ']['. $field->id . ']';
	}

	function get_id( $field )
	{
		return ( isset( $field->settings['options']['is'] ) ) ? $this->prefix .       $field->form_id .  '_'. $field->id .  '_' . $field->settings['options']['is']       : $this->prefix .       $field->form_id .  '_'. $field->id;
	}

	function get_maptype( $maptype )
	{
		...
		if ( isset( $m[$maptype] ) ) return $m[$maptype];
	...
	function submitted( $field )
	{
		...
		$args = array();
		$args['center'] = $value['center_lat'] . ',' . $value['center_lng'];
		$args['zoom'] = $value['zoomlevel'];
		$args['size'] = $width . 'x' . $height;
		$args['maptype'] = $this->get_maptype( $value['maptype'] );
		$args['markers'] = $value['lat'] . ',' . $value['lng'];
		$args['sensor'] = 'false';
		...
		$field->submitted['map']   = add_query_arg( $args, 'http://maps.googleapis.com/maps/api/staticmap' );
