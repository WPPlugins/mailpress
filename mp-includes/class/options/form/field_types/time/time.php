...
class MP_Form_field_type_time extends MP_form_field_type_
{
	...
	{
		return $this->prefix.'['.$field->form_id . ']['. $field->id . ']' .  ( ( isset( $field->settings['options']['is'] ) ) ? ( '[' . ( ( $field->settings['options']['is'] == 'am_pm' ) ? $this->prefix . $field->settings['options']['is'] : $field->settings['options']['is'] ) . ']' ) : '' ) ;
	}

	function get_id( $field )
	{
		return $this->prefix  .  $field->form_id . '_' . $field->id .        ( ( isset( $field->settings['options']['is'] ) ) ? ( '_' . ( ( $field->settings['options']['is'] == 'am_pm' ) ? $field->settings['attributes']['value']           : $field->settings['options']['is'] )       ) : '' ) ;
	}

	public static function valid_date( $y, $m, $d )
	{
		$maxd = array( 31,( !( $y%4 ) && ( $y%100 || !( $y%400 ) ) ) ? 29 : 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 );
		if ( $d > $maxd[$m - 1] ) return false;
		return true;
	}
...
new MP_Form_field_type_time( __( 'Time', 'MailPress' ) );