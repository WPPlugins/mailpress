...
class MP_Form_field_type_captcha_gd1_cryptogra
{
	function __construct()
	{
		...
		$xsettings = __DIR__ . "/cfg/$xstyle/settings.php";
...
//		$_SESSION['cryptogra']['use']++;
	...
	function create( $style, $settings )
	{
		...
		for ( $i = 1; $i <= $charnb; $i++ ) 
		{
			...
			imagettftext( $imgtmp, $tword[$i]['size'], $tword[$i]['angle'], $x, $tword[$i]['y'], $black, __DIR__ . '/fonts/' . $tword[$i]['font'], $tword[$i]['element'] );
		...
		$img = imagecreatetruecolor( $cryptwidth, $cryptheight ); 
		 ...
		 if ( $bgimg ) $bgimg = __DIR__ . "/cfg/$style/$bgimg";
	...
	function ecriture()				// Création de l'écriture
	{
		...
		{       
			...
			imagettftext( $img, $tword[$i]['size'], $tword[$i]['angle'], $x, $tword[$i]['y'], ( $charcolorrnd ) ? $rndink : $ink, __DIR__ . '/fonts/' . $tword[$i]['font'], $tword[$i]['element'] );
