...
//// Columns ////
	...
	public static function columns_list( $id = true )
	{
		...
		foreach ( $columns as $key => $display_name ) 
		{
			$tag  = ( 'cb' === $key ) ? 'td' : 'th';
			$thid = ( $id ) ? ' id="' . $key . '"' : '';
			$hidd = ( in_array( $key, $hidden ) ) ? ' hidden' : '';
			$class = ( 'cb' === $key ) ? ' class="check-column"' : ' class="manage-column column-' . $key . $hidd . '"';


			echo "<$tag$thid$class>$display_name</$tag>";
		} 
	}
...
//// List ////
	...
	public static function pagination( $args, $which = '' ) 
	{
		...
		$page_links[] = sprintf( '<a class="%s" title="%s" href="%s">%s</a>',
			...
			esc_attr( __( 'Go to the first page' ) ),
		...
		$page_links[] = sprintf( '<a class="%s" title="%s" href="%s">%s</a>',
			...
			esc_attr( __( 'Go to the previous page' ) ),
		...
		$page_links[] = sprintf( '<a class="%s" title="%s" href="%s">%s</a>',
			...
			esc_attr( __( 'Go to the next page' ) ),
		...
		$page_links[] = sprintf( '<a class="%s" title="%s" href="%s">%s</a>',
			...
			esc_attr( __( 'Go to the last page' ) ),
...
?>
				...
				<input type="submit" name="do<?php echo $name; ?>" id="do<?php echo $name; ?>" class="button-secondary apply" value="<?php echo esc_attr( __( 'Apply' ) ); ?>" />
<?php
	}
