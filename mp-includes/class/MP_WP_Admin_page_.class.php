	...
	public static function post_url_parms( $url_parms, $parms = array( 'mode', 'status', 's', 'paged', 'author' ) )
	{
		foreach ( $parms as $key )
			if ( isset( $url_parms[$key] ) )
				echo '<input type="hidden" name="' . $key . '" value="' . esc_attr( $url_parms[$key] ) . '" />';
	}
