...
////  Build it  ////
	...
	//¤ mail empty ? ¤//
		if ( !$this->row->subject && !$this->row->plaintext && !$this->row->html && !$this->mail->attachments )
		{
			$this->trace->log( __( 'MAILPRESS [WARNING] - Mail is empty', 'MailPress' ) );
			return false;
		}
...
//SwiftMailer DKIM part----------------------

$privateKey = "-----BEGIN RSA PRIVATE KEY-----
MIIEpAIBAAKCAQEAzBniL+w/ieGffe+/Zg47iqLfJIdCpuBNbn6blq6j03XLmepF
VpsklK8IMD2opypoCGLr2s1kgcCy4Mo6Dd3scEmswWEI2V1xYe99oR9g6Str8GBO

******bla bla blaaaaaa*********

9UFGpYLEQtBFsYLyGK2rsJeANTihnHym1cqiH5LnHHwZ94CsqSv4JvBh6HQIroli
zsgpOD6TmZ4ca03xV+DQBTOlGXwYCc/wB1jMFBJZ294F1qsNN9/w1g==
-----END RSA PRIVATE KEY-----";
$domainName = 'MyDomain.com';
$selector = 'Key1';
$signer = Swift_Signers_DKIMSigner::newInstance( $privateKey, $domainName, $selector );
$this->message->attachSigner( $signer );
	...
	*/
	...
	//¤ Swift connection ¤//
		...
		if ( isset( $this->mail->mailpress_batch_spool_send ) && $this->mail->mailpress_batch_spool_send ) 		new MP_Swift_Connection_spool();
	...
	//¤ message ¤//
		...
		$this->message->setEncoder( Swift_Encoding::getQpEncoding() );
...
//// attachments
	...
	public static function get_attachment_link( $meta, $mail_status )
	{
		$meta_value = unserialize( $meta['meta_value'] );
