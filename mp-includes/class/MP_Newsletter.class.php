...
// for newsletters
	...
	public static function get_active_by_scheduler( $scheduler ) 
	{
		...
		{
			if ( $v['allowed'] && $scheduler == $v['scheduler']['id'] )
			{
				if ( self::as_subscribers( $k ) )
				{
					$x[$k] = $v;
				}
			}
...
////  Object  ////
	...
	public static function as_subscribers( $id ) 
	{
		global $wpdb, $mp_registered_newsletters;

		if ( !isset( $mp_registered_newsletters[$id] ) ) return false;

		$in = ( $mp_registered_newsletters[$id]['default'] ) ? 'NOT' : '';

		return $wpdb->get_var( self::get_query_newsletter( $id, $in, true ) );
	}

	public static function get_query_newsletter( $id, $in = 'NOT', $count = false ) 
	{
		global $wpdb;

		$cols = ($count) ? 'count(*)' : 'DISTINCT a.id, a.email, a.name, a.status, a.confkey';

		return $wpdb->prepare( "SELECT $cols FROM $wpdb->mp_users a 
						WHERE status = 'active' 
						AND 	$in EXISTS 	( 
