...
//// Recipients queries ////
	...
	{
		if ( __CLASS__ == $optgroup ) return __( 'Subscribers', 'MailPress' );
		return $label;
	}
...
/// MAIL URLs ///
	...
	private static function _get_url( $action, $key ){
		...
		{
			case 'ajax':
				return add_query_arg( array( 'action' => 'mp_mlinks', $action => $key ), admin_url( 'admin-ajax.php' ) );
			break;
			case 'page_id':
