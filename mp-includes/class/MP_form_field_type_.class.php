...
*/
	...
	{
		return ( isset( $field->settings['attributes']['name'] ) ) ?  $this->prefix.'['.$field->form_id.']['.$this->prefix.$field->settings['attributes']['name'].']'	: $this->prefix.'['.$field->form_id . ']['. $field->id . ']';
	}

	function get_id( $field )
	{
		return ( isset( $field->settings['attributes']['id'] ) )   ?  $this->prefix  .  $field->form_id.'_'. $field->settings['attributes']['id'] 					: $this->prefix  .  $field->form_id . '_' . $field->id; 
	}
	...
	function settings_form( $field )
	{
		...
		if ( isset( $_GET['action'] ) && ( 'edit' == $_GET['action'] ) && $this->type_ok ) { $tabs['html'] = __( 'Html', 'MailPress' ); }
