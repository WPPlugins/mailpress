var autosave = {
	...
	init : function() {
		if (jQuery('#autosavenonce').val())
		{
			...
			autosave.click_attachments_loader();								// Autosave when the change loader link is clicked.
...
// preview
	...
	display_preview : function () {
		...
		if (selectedtheme != '')
		{
			var TB_iframe = href.indexOf('&TB_iframe');
			hrefnew = href.substring(0,TB_iframe)+'&theme='+selectedtheme+href.substring(TB_iframe);
			href = hrefnew;
		}
...
// attachments
	click_attachments_loader : function() {
		jQuery('#mp_loader_link').click(function(e) {
			autosave.clean_attachments();
			autosave.main();
			return true;
		});
	},

	enable_attachments : function(item_id) {
		draft_id = item_id;
...
// ajax
			jQuery.ajax({
				...
				success: function() {jQuery('#attachment-item-'+ meta_id).remove();jQuery('#attachment-item-u-'+ meta_id).remove();}
			});
		});
	},
...
// html2txt
	...
	click_html2txt : function() {
		jQuery('a#html2txt').click(function() {
			...
			var html2txt_data = {	action: 		"mp_ajax",
						mp_action: "html2txt",
...
// ajax
				...
				if ( '' != res.responses[0].supplemental['tipe'] ) 
				{
					...
					if (item_id > 0)
					{
						...
						jQuery.post(autosaveL10n.requestFile, 
								{action: "mp_ajax", mp_action : "get_previewlink", id: item_id, main_id: item_main_id, getpreviewlinknonce: jQuery('#getpreviewlinknonce').val(), height : parseInt(jQuery(window).height() * .8), width : parseInt(jQuery(window).width()  * .9) }, 
	...
	update_id : function(item_id, type) {
		...
		autosave.enable_attachments(item_id);
	},
