...
wpList = {
	...
	ajaxAdd: function( e, s ) {
		...
		s.data = $.param( $.extend( { _ajax_nonce: s.nonce, action: s.action, mp_action: s.mp_action }, wpAjax.unserialize( cls[4] || '' ) ) );
	...
	ajaxDel: function( e, s ) {
		...
		s.action = 'mp_ajax';
		s.mp_action = 'delete_' + s.what;

		s.nonce = wpList.nonce(e,s);

		s.data = $.extend(
			{ action: s.action, mp_action: s.mp_action, id: s.element.split('-').pop(), _ajax_nonce: s.nonce },
	...
	ajaxDim: function( e, s ) {
		...
		s.action = 'mp_ajax';
		s.mp_action = 'dim_' + s.what;

		s.nonce = wpList.nonce(e,s);

		s.data = $.extend(
			{ action: s.action, mp_action: s.mp_action, id: s.element.split('-').pop(), dimClass: s.dimClass, _ajax_nonce : s.nonce },
